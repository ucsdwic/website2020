export const all_members = [
  {
    //picture: require("./demoPic.jpg")
    picture: require("../static/board-members/vas-sengupta.jpg"),
    name: "Vas Sengupta",
    position: "President",
    year: "Class of 2022",
    linkedin: "https://www.linkedin.com/in/vsengupta/",
    instagram: "https://www.instagram.com/vas.sengupta/",
    github: "",
    bio: "Hi everyone! My name is Vas Sengupta and I’m WIC’s Co-President for this year! I’m a fourth year Computer Engineering student from Warren College. This past summer I interned at Microsoft and visited Seattle. I had an amazing time in the PNW exploring coffee shops, visiting tourist attractions, and enjoying the scenery! This year, I’m looking forward to growing the WIC community and raising awareness about WIC’s mission!"
  },
  {
    picture: require("../static/board-members/angela-wang.jpg"),
    name: "Angela Wang",
    position: "President",
    year: "Class of 2022",
    linkedin: "https://linkedin.com/in/wangangela2000/",
    instagram: "",
    github: "",
    bio: "Hello! My name is Angela Wang and I’m a current senior in Muir College majoring in Math-CS and minoring in Design. I’m currently the Co-President of WIC and have been a part of WIC for the past few years. A fun fact about myself is that I dyed my hair 5+ times over quarantine."
  },
  {
    picture: require("../static/board-members/sydney-wong.jpg"),
    name: "Sydney Wong",
    position: "Vice President Internal",
    year: "Class of 2023",
    linkedin: "https://www.linkedin.com/in/sydney-wong-269a83193",
    instagram: "https://www.instagram.com/sydneywong8/",
    github: "",
    bio: "Hi all! My name is Sydney, and I am a third year computer science major in Sixth college. This summer I interned at Intuit and visited Madison, Wisconsin. In my spare time, I love to play tennis, read, spend time with my dog, and watch Law & Order! As this year’s Vice President Internal, I am super excited to continue growing the WIC community and empowering women in STEM. I can’t wait to meet you all during our WIC events!"
  },
  {
    picture: require("../static/board-members/navika-budhraja.JPG"),
    name: "Navika Budhraja",
    position: "Vice President External",
    year: "Class of 2023",
    linkedin: "",
    instagram: "",
    github: "",
    bio: ""
  },
  {
    picture: require("../static/board-members/default-img.jpg"),
    name: "Sarayu Pai",
    position: "Vice President External",
    year: "",
    linkedin: "",
    instagram: "",
    github: "",
    bio: ""
  },
  {
    picture: require("../static/board-members/jaz-gill.jpg"),
    name: "Jaz Gill",
    position: "Treasurer",
    year: "Class of 2022",
    linkedin: "",
    instagram: "",
    github: "",
    bio: "My name is Jaz and I’m a 4th year Computer Science Major with a minor in Cognitive Science. I’m in Thurgood Marshall College and my pronouns are she/her/hers. In my free time I love to travel and eat :)"
  },
  {
    picture: require("../static/board-members/lindsey-manzano.jpg"),
    name: "Lindsey Manzano",
    position: "Secretary",
    year: "Class of 2024",
    linkedin: "https://www.linkedin.com/in/lindsey-manzano-886a7316",
    instagram: "https://www.instagram.com/lindseymanzano/",
    github: "https://github.com/lindseymanzano",
    bio: "Hello! My name is Lindsey Manzano and I'm a 2nd year studying computer science in Marshall College! I enjoy dancing, playing soccer, hiking, listening to music, going to the beach, and spending time with friends and family. Fun fact: I danced at a Dance Moms competition and saw Abby Lee Miller (and Christi and Kelly fighting)."
  },
  {
    picture: require("../static/board-members/judy-liu.JPG"),
    name: "Judy Liu",
    position: "Technical Development Co-Chair",
    year: "Class of 2023",
    linkedin: "",
    instagram: "",
    github: "",
    bio: "My name is Judy Liu and I’m a second year Computer Science major in Warren college. My favourite thing to do is to explore delicious foods from different cultures. I love to meet new people and help them to the best of my abilities."
  },
  {
    picture: require("../static/board-members/aarushi-shah.jpg"),
    name: "Aarushi Shah",
    position: "Technical Development Co-Chair",
    year: "Class of 2022",
    linkedin: "",
    instagram: "",
    github: "",
    bio: "Hi everyone, my name is Aarushi Shah and I am one of the Technical Development Chairs this year! I am a 3rd year Computer Science major in Warren :) In addition to WIC, I am also apart of a dance team on campus called UCSD Raas Ruckus. I love dancing and being apart of a team as it is such a great way to make new friends and relieve some stress from school :)"
  },
  {
    picture: require("../static/board-members/lavanya-verma.jpg"),
    name: "Lavanya Verma",
    position: "Technical Development Co-Chair",
    year: "Class of 2023",
    linkedin: "",
    instagram: "",
    github: "",
    bio: "Hi! I’m Lavanya, the Tech co-Chair for Wic. I’m starting my 3rd year as a Computer Science major at Warren College. This summer I finally visited Harry Potter world, hiked 6 miles at Grand Canyon and kayaked."
  },
  {
    picture: require("../static/board-members/jaida-day.jpg"),
    name: "Jaida Day",
    position: "Web Development Chair",
    year: "Class of 2022",
    linkedin: "https://www.linkedin.com/in/jaida-day/",
    instagram: "",
    github: "https://github.com/aadiaaj",
    bio: "Hi everyone! My name is Jaida Day and I’m a senior majoring in Math-CS. I’m from Revelle College #firstandfinest and in my free time I like to dance and/or hang with family and friends. Over the summer I interned with PricewaterhouseCoopers, visited a lot of view spots, and hung out with some dolphins!"
  },
  {
    picture: require("../static/board-members/harshi-agrawal.jpg"),
    name: "Harshi Agrawal",
    position: "Social Co-Chair",
    year: "Class of 2022",
    linkedin: "https://www.linkedin.com/in/harshiagrawal/",
    instagram: "https://www.instagram.com/harshiagrawal/?hl=en",
    github: "",
    bio: "Hey my name is Harshi. I’m a senior majoring in Computer science and minoring in Business in Warren college. I’m super excited to be back in person and hopefully meet more of you! This summer I interned at Qualcomm, spent a lot of time with friends and family, and did some traveling!"
  },
  {
    picture: require("../static/board-members/lily-steiner.jpg"),
    name: "Lily Steiner",
    position: "Social Co-Chair",
    year: "Class of 2023",
    linkedin: "",
    instagram: "",
    github: "",
    bio: "Hi everyone! My name is Lily and I’m a 3rd year CS Bioinformatics major in Warren. This summer I did research at a lab where I worked on accelerating bioinformatics tools. I spent my free time out with friends, cooking, and traveling! My summer obsession has been animal crossing and collecting vinyl :)"
  },
  {
    picture: require("../static/board-members/nora-yinxuan-du.jpg"),
    name: "Nora Yinxuan Du",
    position: "BPC Co-Chair",
    year: "Class of 2023",
    linkedin: "",
    instagram: "",
    github: "",
    bio: "My name is Nora Yinxuan Du. I am a junior majoring in CS and I am from Warren College. I moved to the South Bay this summer and had a great experience interning at Salesforce. A fun fact is that I took advantage of volunteering time-off and gave back to the community. The most exciting part for me was volunteering with my fellow interns on a beach cleanup in San Francisco, which took me 1.5 hours on the Caltrain to get there."
  },
  {
    picture: require("../static/board-members/tri-truong.jpg"),
    name: "Tri Truong",
    position: "BPC Co-Chair",
    year: "Class of 2022",
    linkedin: "",
    instagram: "",
    github: "",
    bio: "Hey everyone! My name is Tri, and I’m WIC’s co-Beginner’s Programming Competition (BPC) chair. I’m a 4th year CS major in Warren. Outside of coding, I play a lot of sports (soccer, basketball, swimming, etc.), but I don’t really get that great at any of them L. I also enjoy working out in the gym. When I don’t study or exercise, I usually read/watch superheroes stuff and watch K-dramas."
  },
  {
    picture: require("../static/board-members/sravya-balasa.jpg"),
    name: "Sravya Balasa",
    position: "Outreach Co-Chair",
    year: "Class of 2022",
    linkedin: "",
    instagram: "",
    github: "https://github.com/sravyabalasa",
    bio: "Hi everyone, my name is Sravya Balasa and I'm WIC's Outreach Co-Chair! I'm a 4th year Computer Science major, Design minor, in ERC :) After the stressful past year, I wanted to use this summer to put more time into what I loved. When I wasn't doing my internship at Salesforce, I did many photography shoots, wrote more on my Medium blog, painted, and traveled! I went to Seattle and Kauai and it was so green and beautiful."
  },
  {
    picture: require("../static/board-members/vivian-wu.jpg"),
    name: "Vivian Wu",
    position: "Outreach Co-Chair",
    year: "Class of 2023",
    linkedin: "",
    instagram: "",
    github: "",
    bio: "Hi! I'm a third year Computer Science major and Cognitive Science minor in Warren! A fun fact about me is that I play Table Tennis for ucsd and for fun. Over the summer, I did a lot of exercising, knitting, and embroidering."
  },
  {
    picture: require("../static/board-members/claudia-handoyo.jpg"),
    name: "Claudia Handoyo",
    position: "Public Relations-Publicity Co-Chair",
    year: "Class of 2022",
    linkedin: "",
    instagram: "",
    github: "",
    bio: "Hi everyone! My name is Claudia and I’m WIC’s PR-Publicity Chair this year. I’m a 4th year in ERC studying Math-CS. I enjoy cooking, eating, watching movies/dramas/anime, and traveling! A fun fact about me is that I usually order my bubble tea drinks with no sugar and no ice. Looking forward to meeting all of you this year!"
  },
  {
    picture: require("../static/board-members/vivian-dang.jpg"),
    name: "Vivian Dang",
    position: "Public Relations-Marketing Co-Chair",
    year: "Class of 2023",
    linkedin: "",
    instagram: "",
    github: "",
    bio: "Hi ya’ll! I’m Vivian, this year’s PR-Marketing Chair. I’m a third year majoring in Math-Computer Science (please hit me up if you’re also math-cs) from Revelle College. I like watching Netflix and working out in my free time. Over the summer I interned at General Atomics (with Navika, one of the VPE co-chairs), hiked and ran a lot, and got to spend time with friends and family <3"
  },
];
